...
impl Chain {
    ...
    pub fn new(
        ...
        let genesis_block_hash = storage.block_hash(0).expect(
            "storage with genesis block is required",
        );
        ...
        let is_segwit_active = deployments.segwit(
            best_storage_block.number,
    ...
    pub fn block_number(&self, hash: &H256) -> Option<BlockHeight> {
        ...
        self.headers_chain.height(hash).map(|p| {
            self.best_storage_block.number + p + 1
        })
    }
    ...
    pub fn block_header_by_number(&self, number: BlockHeight) -> Option<IndexedBlockHeader> {
        if number <= self.best_storage_block.number {
            ...
            self.headers_chain.at(
                number - self.best_storage_block.number,
            )
        }
    }
    ...
    pub fn block_state(&self, hash: &H256) -> BlockState {
        match self.hash_chain.contains_in(hash) {
            ...
            None => {
                if self.storage.contains_block(
                    storage::BlockRef::Hash(hash.clone()),
                )
                {
                    BlockState::Stored
    ...
    pub fn insert_best_block(
        ...
        match block_origin {
            ...
            storage::BlockOrigin::CanonChain { .. } => {
                ...
                self.is_segwit_active = self.deployments.segwit(
                    self.best_storage_block.number,
                ...
                self.headers_chain.block_inserted_to_storage(
                    block.hash(),
                    &self.best_storage_block.hash,
                );
            ...
            storage::BlockOrigin::SideChainBecomingCanonChain(origin) => {
                ...
                let new_main_blocks_transactions_hashes = origin
                    ...
                    .into_iter()
                    .flat_map(|block_hash| {
                        self.storage.block_transaction_hashes(block_hash.into())
                    })
                    .collect::<Vec<_>>();

                let mut memory_pool = self.memory_pool.write();
                for transaction_accepted in
                    this_block_transactions_hashes.into_iter().chain(
                        new_main_blocks_transactions_hashes.into_iter(),
                    )
                {
    ...
    pub fn forget_block_leave_header(&mut self, hash: &H256) -> HashPosition {
        match self.hash_chain.remove_at(VERIFYING_QUEUE, hash) {
            HashPosition::Missing => {
                match self.hash_chain.remove_at(REQUESTED_QUEUE, hash) {
            ...
            for h in all_keys {
                let remove_verifying_transaction = {
                    if let Some(entry) = self.verifying_transactions.get(&h) {
                        if entry.raw.inputs.iter().any(
                            |i| i.previous_output.hash == hash,
                        )
                        {
    ...
    pub fn transaction_by_hash(&self, hash: &H256) -> Option<IndexedTransaction> {
        self.verifying_transactions.get(hash).cloned().or_else(|| {
            self.memory_pool.read().read_by_hash(hash).cloned().map(
                |t| {
                    t.into()
                },
            )
    ...
    fn block_locator_hashes_for_storage(
        ...
        loop {
            let block_hash = self.storage.block_hash(index).expect(
                "private function; index calculated in `block_locator_hashes`; qed",
            );
            hashes.push(block_hash);
...
impl storage::TransactionProvider for Chain {
    fn transaction_bytes(&self, hash: &H256) -> Option<Bytes> {
        self.memory_pool.read().transaction_bytes(hash).or_else(
            || {
                self.storage.transaction_bytes(hash)
            },
        )
    }

    fn transaction(&self, hash: &H256) -> Option<Transaction> {
        self.memory_pool.read().transaction(hash).or_else(|| {
            self.storage.transaction(hash)
        })
    }
}
...
mod tests {
    ...
    #[test]
    fn chain_empty() {
        let db = Arc::new(BlockChainDatabase::init_test_chain(
    ...
    fn chain_block_path() {
        ...
        assert!(
            chain.information().scheduled == 6 && chain.information().requested == 0 &&
        ...
        assert!(
            chain.information().scheduled == 4 && chain.information().requested == 2 &&
                chain.information().verifying == 0 && chain.information().stored == 1
        );
        ...
        assert!(
            chain.information().scheduled == 3 && chain.information().requested == 3 &&
                chain.information().verifying == 0 && chain.information().stored == 1
        );
        ...
        assert!(
            chain.information().scheduled == 3 && chain.information().requested == 1 &&
                chain.information().verifying == 0 && chain.information().stored == 1
        );
    ...
    fn chain_block_locator_hashes() {
        ...
        chain.insert_best_block(block1.into()).expect(
            "Error inserting new block",
        );
        assert_eq!(
        ...
        chain.insert_best_block(block2.into()).expect(
            "Error inserting new block",
        );
        assert_eq!(
            chain.block_locator_hashes(),
    ...
    fn chain_transaction_state() {
        ...
        assert_eq!(
            chain.transaction_state(&block1.transactions[0].hash()),
            TransactionState::Unknown
        );
        assert_eq!(
    ...
    fn chain_forget_verifying_transaction_with_children() {
        ...
        assert!(!chain.forget_verifying_transaction(
            &test_chain.at(0).hash(),
        ));
        assert!(!chain.forget_verifying_transaction(
            &test_chain.at(1).hash(),
        ));
        assert!(!chain.forget_verifying_transaction(
            &test_chain.at(2).hash(),
        ));
    ...
    fn memory_pool_transactions_are_reverified_after_reorganization() {
        ...
        assert!(result.transactions_to_reverify.iter().any(|ref tx| {
            &tx.hash == &tx1_hash
        }));
        assert!(result.transactions_to_reverify.iter().any(|ref tx| {
            &tx.hash == &tx2_hash
    ...
    fn fork_chain_block_transaction_is_removed_from_on_block_insert() {
        ...
        assert_eq!(
            chain.insert_best_block(b0.clone().into()).expect(
                "block accepted",
            ),
        ...
        assert_eq!(
            chain.insert_best_block(b1.clone().into()).expect(
                "block accepted",
            ),
        ...
        assert_eq!(chain.information().transactions.transactions_count, 3);
        assert_eq!(
            chain.insert_best_block(b2.clone().into()).expect(
                "block accepted",
            ),
        ...
        assert_eq!(chain.information().transactions.transactions_count, 3);
        assert_eq!(
            chain.insert_best_block(b3.clone().into()).expect(
                "block accepted",
            ),
        ...
        assert_eq!(chain.information().transactions.transactions_count, 3);
        assert_eq!(
            chain.insert_best_block(b4.clone().into()).expect(
                "block accepted",
            ),
            BlockInsertionResult::default()
        );
        assert_eq!(chain.information().transactions.transactions_count, 3);
        // order matters
        let insert_result = chain.insert_best_block(b5.clone().into()).expect(
            "block accepted",
        );
...
fn double_spend_transaction_is_removed_from_memory_pool_when_output_is_spent_in_block_transaction(){
        ...
        let tx2: Transaction = test_data::TransactionBuilder::with_output(20)
            .add_input(&tx0, 0)
            .into();
        let tx3: Transaction = test_data::TransactionBuilder::with_output(20)
            .add_input(&tx0, 1)
            .into();
