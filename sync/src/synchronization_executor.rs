...
impl LocalSynchronizationTaskExecutor {
    ...
    fn execute_block(&self, peer_index: PeerIndex, block: IndexedBlock) {
        if let Some(connection) = self.peers.connection(peer_index) {
            ...
            self.peers.hash_known_as(
                peer_index,
                block.hash().clone(),
                KnownHashType::Block,
            );
            let block = types::Block { block: block.to_raw_block() };
    ...
    fn execute_transaction(&self, peer_index: PeerIndex, transaction: IndexedTransaction) {
        if let Some(connection) = self.peers.connection(peer_index) {
            ...
            let transaction = types::Tx { transaction: transaction.raw };
            ...
            match self.peers.filter_block(peer_index, &block) {
                BlockAnnouncementType::SendInventory => {
                    self.execute_inventory(
                        peer_index,
                        types::Inv::with_inventory(
                            vec![InventoryVector::block(block.hash().clone())],
                        ),
                    );
                }
                ...
                BlockAnnouncementType::SendCompactBlock => {
                    if let Some(compact_block) =
                        self.peers.build_compact_block(peer_index, &block)
                    {
    ...
    fn execute_relay_transaction(&self, transaction: IndexedTransaction, fee_rate: u64) {
        for peer_index in self.peers.enumerate() {
            match self.peers.filter_transaction(
                peer_index,
                &transaction,
                Some(fee_rate),
            ) {
                TransactionAnnouncementType::SendInventory => {
                    self.execute_inventory(
                        peer_index,
                        types::Inv::with_inventory(
                            vec![InventoryVector::tx(transaction.hash.clone())],
                        ),
                    )
                }
...
pub mod tests {
    ...
    use synchronization_peers::{PeersImpl, PeersContainer, PeersFilters, PeersOptions,
                                BlockAnnouncementType};
    ...
    fn relay_new_block_after_sendcmpct() {
        ...
        assert_eq!(
            *c1.messages.lock().entry("inventory".to_owned()).or_insert(
                0,
            ),
            1
        );
    ...
    fn relay_new_transaction_with_bloom_filter() {
        ...
        assert_eq!(
            *c2.messages.lock().entry("inventory".to_owned()).or_insert(
                0,
            ),
            0
        );
        assert_eq!(
            *c3.messages.lock().entry("inventory".to_owned()).or_insert(
                0,
            ),
            1
        );
        assert_eq!(
            *c4.messages.lock().entry("inventory".to_owned()).or_insert(
                0,
            ),
            1
        );
