    ...
    TExecutor: TaskExecutor,
{
        ...
        match next_item.inv_type {
            ...
            common::InventoryType::MessageWitnessTx => {
                ...
                if let Some(transaction) = self.memory_pool.read().read_by_hash(&next_item.hash) {
                    ...
                    let transaction = IndexedTransaction::new(next_item.hash, transaction.clone());
                    self.executor.execute(Task::WitnessTransaction(
                        peer_index,
                        transaction,
                    ));
                } else {
                ...
                if let Some(block) = self.storage.block(next_item.hash.clone().into()) {
                    ...
                    if let Some(message_artefacts) = message_artefacts {
                        ...
                        for matched_transaction in message_artefacts.matching_transactions {
                            ...
                            self.executor.execute(Task::Transaction(
                                peer_index,
                                matched_transaction,
                            ));
                        }
            ...
            common::InventoryType::MessageCompactBlock => {
                if let Some(block) = self.storage.block(next_item.hash.clone().into()) {
                    ...
                    if let Some(message) = message {
                        ...
                        self.executor.execute(
                            Task::CompactBlock(peer_index, message),
                        );
                    }
                } else {
                    notfound.inventory.push(next_item);
            ...
            common::InventoryType::MessageWitnessBlock => {
                if let Some(block) = self.storage.block(next_item.hash.clone().into()) {
                    ...
                    self.executor.execute(
                        Task::WitnessBlock(peer_index, block.into()),
                    );
                } else {
                    notfound.inventory.push(next_item);
                }
            ...
            common::InventoryType::MessageWitnessFilteredBlock => (),
        }
    ...
    fn serve_get_blocks(&self, peer_index: PeerIndex, message: types::GetBlocks) {
            ...
            self.locate_best_common_block(&message.hash_stop, &message.block_locator_hashes)
        {
            let inventory: Vec<_> = (block_height + 1..
                                         block_height + 1 +
                                             (types::GETBLOCKS_MAX_RESPONSE_HASHES as BlockHeight))
            ...
            self.peers.misbehaving(
                peer_index,
                "Got 'getblocks' message without known blocks",
            );
            return;
        }
    }
    ...
    fn serve_mempool(&self, peer_index: PeerIndex) {
        let inventory: Vec<_> = self.memory_pool
            .read()
    ...
    fn serve_get_block_txn(&self, peer_index: PeerIndex, message: types::GetBlockTxn) {
        ...
        let block_transactions = self.storage.block_transaction_hashes(
            message.request.blockhash.clone().into(),
        );
        ...
        for transaction_index in message.request.indexes {
            ...
            if let Some(transaction) =
                self.storage.transaction(
                    &block_transactions[transaction_index],
                )
            {
            ...
            loop {
                ...
                if let Some(block_number) =
                    self.storage.block_number(
                        &block_header.previous_header_hash,
                    )
                {
                    return Some(block_number);
                }
...
pub mod tests {
    ...
    impl DummyServer {
        pub fn new() -> Self {
            DummyServer { tasks: Mutex::new(Vec::new()) }
        }
    ...
    fn create_synchronization_server()
        -> (StorageRef, MemoryPoolRef, ExecutorRef<DummyTaskExecutor>, PeersRef, ServerImpl)
    {
    ...
    fn server_getdata_responds_notfound_when_block_not_found() {
        ...
        assert_eq!(
            tasks,
            vec![
                Task::NotFound(0, types::NotFound::with_inventory(inventory)),
            ]
        );
    }
    ...
    fn server_getblocks_responds_inventory_when_have_unknown_blocks() {
        ...
        storage.insert(test_data::block_h1().into()).expect(
            "Db write error",
        );
    ...
    fn server_getheaders_do_not_responds_headers_when_synchronized() {
        ...
        assert_eq!(
            tasks,
            vec![
                Task::Headers(0, types::Headers::with_headers(vec![]), Some(dummy_id)),
            ]
        );
    }
    ...
    fn server_getheaders_responds_headers_when_have_unknown_blocks() {
        ...
        assert_eq!(
            tasks,
            vec![
                Task::Headers(0, types::Headers::with_headers(headers), Some(dummy_id)),
            ]
        );
    }
