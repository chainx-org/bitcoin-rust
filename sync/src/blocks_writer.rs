...
use super::Error;
use synchronization_verifier::{Verifier, SyncVerifier, VerificationTask, VerificationSink,
                               BlockVerificationSink, TransactionVerificationSink};
...
impl BlocksWriter {
    ...
    pub fn append_block(&mut self, block: chain::IndexedBlock) -> Result<(), Error> {
        ...
        if self.storage.contains_block(
            storage::BlockRef::Hash(block.hash().clone()),
        )
        {
            return Ok(());
        }
        ...
        let mut verification_queue: VecDeque<chain::IndexedBlock> =
