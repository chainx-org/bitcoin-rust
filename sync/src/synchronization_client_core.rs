...
use synchronization_verifier::{VerificationSink, BlockVerificationSink,
                               TransactionVerificationSink, VerificationTask};
use types::{BlockHeight, ClientCoreRef, PeersRef, PeerIndex, SynchronizationStateRef,
            EmptyBoxFuture, SyncListenerRef};
use utils::{AverageSpeedMeter, MessageBlockHeadersProvider, OrphanBlocksPool,
            OrphanTransactionsPool, HashPosition};
#[cfg(test)]
    ...
    T: TaskExecutor,
{
    ...
    fn on_headers(&mut self, peer_index: PeerIndex, message: types::Headers) {
        ...
        match self.verify_headers(
            ...
            BlocksHeadersVerificationResult::Error(error_index) => {
                self.chain.mark_dead_end_block(
                    &headers[first_unknown_index + error_index].hash,
                )
            }
    ...
    fn on_block(
        ...
        // update peers to select next tasks
        self.peers_tasks.on_block_received(
            peer_index,
            &block.header.hash,
        );
        ...
        match block_state {
            ...
            BlockState::DeadEnd => {
                ...
                let parent_block_state = self.chain.block_state(
                ...
                match parent_block_state {
                    BlockState::Unknown | BlockState::DeadEnd => {
                        ...
                        if self.state.is_synchronizing() {
                            ...
                            let removed_blocks_hashes: Vec<_> = self.orphaned_blocks_pool
                                ...
                                .map(|b| b.header.hash)
                                .collect();
                            self.chain.forget_blocks_leave_header(
                                &removed_blocks_hashes,
                            );
                        } else {
                    ...
                    BlockState::Verifying | BlockState::Stored => {
                        ...
                        blocks_to_verify.extend(
                            self.orphaned_blocks_pool.remove_blocks_for_parent(
                                &block.header.hash,
                            ),
                        );
                        ...
                        {
                            self.verifying_blocks_by_peer.insert(
                        ...
                        match self.verifying_blocks_futures.entry(peer_index) {
                            Entry::Occupied(mut entry) => {
                                entry.get_mut().0.extend(blocks_to_verify.iter().map(
                                    |b| b.hash().clone(),
                                ));
                            }
                            Entry::Vacant(entry) => {
    ...
    fn accept_transaction(
        ...
        match self.try_append_transaction(transaction.into(), true) {
            Err(AppendTransactionError::Orphan(_)) => Err(
                "Cannot append transaction as its inputs are unknown"
                    .to_owned(),
            ),
            Err(AppendTransactionError::Synchronizing) => Err(
                "Cannot append transaction as node is not yet fully synchronized"
                    .to_owned(),
            ),
            Ok(transactions) => {
    ...
    fn execute_synchronization_tasks(
        ...
        let blocks_idle_peers: Vec<_> = self.peers_tasks
            ...
            .collect();
        {
            ...
            let headers_idle_peers: Vec<_> = self.peers_tasks
            ...
            if !headers_idle_peers.is_empty() {
                ...
                if scheduled_hashes_len < MAX_SCHEDULED_HASHES {
                    ...
                    let block_locator_hashes = self.chain.block_locator_hashes();
                    let headers_tasks =
                        headers_idle_peers.iter().map(move |peer_index| {
                            Task::GetHeaders(
                                *peer_index,
                                types::GetHeaders::with_block_locator_hashes(
            ...
            if blocks_idle_peers_len != 0 {
                ...
                if requested_hashes_len != 0 {
                    ...
                    let synchronization_queue_will_be_full_in =
                        if synchronization_speed < 0.01_f64 {
                    ...
                    if synchronization_queue_will_be_full_in >
                        ...
                        let hashes_requests_to_duplicate_len =
                            (synchronization_speed *
                                 (synchronization_queue_will_be_full_in -
                                      verification_queue_will_be_empty_in)) as
                                BlockHeight;
...
impl<T> VerificationSink for CoreVerificationSink<T>
where
    T: TaskExecutor,
{
}
    ...
    T: TaskExecutor,
{
    ...
    fn on_transaction_verification_success(&self, transaction: IndexedTransaction) {
        self.core.lock().on_transaction_verification_success(
            transaction,
        )
    }
    ...
    fn on_transaction_verification_error(&self, err: &str, hash: &H256) {
        self.core.lock().on_transaction_verification_error(
            err,
            hash,
        )
    }
    ...
    T: TaskExecutor,
{
    ...
    fn process_peer_transaction(
        ...
        match self.try_append_transaction(transaction.clone(), relay) {
            Err(AppendTransactionError::Orphan(unknown_parents)) => {
                self.orphaned_transactions_pool.insert(
                    transaction,
                    unknown_parents,
                );
                None
    ...
    fn prepare_blocks_requests_tasks(
        ...
        let mut tasks: Vec<Task> = Vec::new();
        let is_segwit_active = self.chain.is_segwit_active();
        let inv_type = if is_segwit_active {
    ...
    fn switch_to_saturated_state(&mut self) {
        ...
        // leave currently verifying blocks
        {
            ...
            self.chain.forget_all_blocks_with_state(
                BlockState::Requested,
            );
            self.chain.forget_all_blocks_with_state(
    ...
    fn on_block_verification_success(
        ...
        match {
            ...
            if self.chain.forget_block_with_state_leave_header(
                block.hash(),
                BlockState::Verifying,
            ...
            Ok(insert_result) => {
                // update shared state
                self.shared_state.update_best_storage_block_height(
                    self.chain.best_storage_block().number,
                );
                ...
                if !segwit_was_active && self.chain.is_segwit_active() {
                    self.peers.require_peer_services(
                        Services::default().with_witness(true),
                    );
                }
                ...
                for tx in insert_result.transactions_to_reverify {
                    ...
                    if let Some(tx_orphans) = self.process_peer_transaction(
                        None,
                        tx.into(),
                        false,
                    )
                    {
                        let tx_tasks = tx_orphans.into_iter().map(|tx| {
                            VerificationTask::VerifyTransaction(next_block_height, tx)
                        });
...
pub mod tests {
    ...
    fn create_sync(
        ...
        let storage = match storage {
            ...
            None => Arc::new(BlockChainDatabase::init_test_chain(
                vec![test_data::genesis().into()],
            )),
        };
        let sync_state =
    ...
    fn request_blocks(peer_index: PeerIndex, hashes: Vec<H256>) -> Task {
        Task::GetData(
            peer_index,
            types::GetData { inventory: hashes.into_iter().map(InventoryVector::block).collect() },
        )
    }
    ...
    fn synchronization_parallel_peers() {
...

        {
            ...
            let information = core.lock().information();
            assert!(
                information.chain.requested == 0 && information.orphaned_blocks == 0 &&
    ...
    fn synchronization_asks_for_inventory_after_saturating() {
        ...
        assert!(tasks.iter().any(|t| {
            t == &request_block_headers_genesis_and(1, vec![block.hash()])
        }));
        assert!(tasks.iter().any(|t| {
            t == &request_block_headers_genesis_and(2, vec![block.hash()])
        }));
        assert!(tasks.iter().any(|t| t == &Task::MemoryPool(1)));
        assert!(tasks.iter().any(|t| t == &Task::MemoryPool(2)));
        assert!(tasks.iter().any(|t| {
            t == &Task::RelayNewBlock(block.clone().into())
        }));
    }
    ...
    fn synchronization_remembers_correct_block_headers_in_order() {
        ...
        assert!(tasks.iter().any(|t| {
            t == &request_block_headers_genesis_and(1, vec![b2.hash(), b1.hash()])
        }));
        assert!(tasks.iter().any(|t| {
            t == &request_blocks(1, vec![b1.hash(), b2.hash()])
        }));
    ...
    fn do_not_rerequest_unknown_block_in_inventory() {
        ...
        assert_eq!(
            ...
            vec![
                Task::GetData(
                    1,
                    types::GetData::with_inventory(
                        vec![InventoryVector::block(test_data::block_h1().hash())],
                    )
                ),
            ]
        );
    ...
    fn peer_not_removed_from_sync_after_responding_with_non_requested_block_notfound() {
        ...
        sync.on_notfound(
            1,
            types::NotFound::with_inventory(
                vec![InventoryVector::block(test_data::block_h170().hash())],
            ),
        );
    ...
    fn transaction_is_requested_when_not_synchronizing() {
...

        {
            ...
            assert_eq!(
                ...
                vec![
                    Task::GetData(
                        0,
                        types::GetData::with_inventory(vec![InventoryVector::tx(H256::from(0))])
                    ),
                ]
            );
        }
        ...
        sync.on_inventory(
            0,
            types::Inv::with_inventory(vec![InventoryVector::tx(H256::from(1))]),
        );
        ...
        assert_eq!(
            ...
            vec![
                Task::GetData(
                    0,
                    types::GetData::with_inventory(vec![InventoryVector::tx(H256::from(1))])
                ),
            ]
        );
    }
    ...
    fn known_transaction_is_not_requested() {
        ...
        sync.on_inventory(
            ...
            types::Inv::with_inventory(vec![
                InventoryVector::tx(
                    test_data::genesis().transactions[0].hash()
                ),
    ...
    fn relay_new_block_when_in_saturated_state() {
        ...
        // we were in nearly saturated state => block is relayed
        {
            ...
            assert!(tasks.iter().any(
                |t| t == &Task::RelayNewBlock(b3.clone().into()),
            ));
        }
    }
