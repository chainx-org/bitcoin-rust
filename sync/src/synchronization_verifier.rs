...
use verification::{BackwardsCompatibleChainVerifier as ChainVerifier,
                   Verify as VerificationVerify, Error as VerificationError, VerificationLevel};
...
impl ChainVerifierWrapper {
    ...
    pub fn new(
        ...
        let enforce_full_verification = AtomicBool::new(storage.contains_block(
            verification_params.verification_edge.clone().into(),
        ));
    ...
    pub fn verify_block(&self, block: &IndexedBlock) -> Result<(), VerificationError> {
        let enforce_full_verification =
            if block.hash() == &self.verification_params.verification_edge {
                self.enforce_full_verification.store(
                    true,
                    Ordering::Relaxed,
                );
                true
...
impl Verifier for AsyncVerifier {
    ...
    fn verify_block(&self, block: IndexedBlock) {
        self.verification_work_sender
            ...
            .expect(
                "Verification thread have the same lifetime as `AsyncVerifier`",
            );
    }
    ...
    T: VerificationSink,
{
    ...
    fn verify_block(&self, block: IndexedBlock) {
        match self.verifier.verify_block(&block) {
            ...
            Err(e) => {
                self.sink.on_block_verification_error(
                    &format!("{:?}", e),
                    block.hash(),
                )
            }
...
pub mod tests {
    ...
    use verification::{VerificationLevel, BackwardsCompatibleChainVerifier as ChainVerifier,
                       Error as VerificationError, TransactionError};
    use synchronization_client_core::CoreVerificationSink;
    use synchronization_executor::tests::DummyTaskExecutor;
    use primitives::hash::H256;
    use chain::{IndexedBlock, IndexedTransaction};
    use super::{Verifier, BlockVerificationSink, TransactionVerificationSink, AsyncVerifier,
                VerificationTask, ChainVerifierWrapper};
    use types::{BlockHeight, StorageRef, MemoryPoolRef};
    ...
    impl Verifier for DummyVerifier {
        fn verify_block(&self, block: IndexedBlock) {
            match self.sink {
                Some(ref sink) => {
                    match self.errors.get(&block.hash()) {
        ...
        fn verify_transaction(&self, _height: BlockHeight, transaction: IndexedTransaction) {
            match self.sink {
                Some(ref sink) => {
                    match self.errors.get(&transaction.hash) {
                        Some(err) => {
                            sink.on_transaction_verification_error(&err, &transaction.hash)
                        }
                        None => {
    ...
    fn verifier_wrapper_switches_to_full_mode() {
        let storage: StorageRef = Arc::new(BlockChainDatabase::init_test_chain(
