...
impl Peers for PeersImpl {
    fn require_peer_services(&self, services: Services) {
        ...
        for peer_index in peers
            ...
            .collect::<Vec<_>>()
        {
            let peer = peers.remove(&peer_index).expect(
                "iterating peers keys; qed",
            );
    ...
    fn connection(&self, peer_index: PeerIndex) -> Option<OutboundSyncConnectionRef> {
        self.peers.read().get(&peer_index).map(|peer| {
...
impl PeersFilters for PeersImpl {
    ...
    fn filter_transaction(
        ...
        if let Some(peer) = self.peers.read().get(&peer_index) {
            if peer.filter.filter_transaction(
                transaction,
    ...
    fn build_compact_block(
        ...
        self.peers.read().get(&peer_index).map(|peer| {
            peer.filter.build_compact_block(block)
        })
    }
    ...
    fn build_merkle_block(
        ...
        self.peers.read().get(&peer_index).and_then(|peer| {
            peer.filter.build_merkle_block(block)
        })
    }
}
