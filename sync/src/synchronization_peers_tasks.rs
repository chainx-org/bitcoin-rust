...
impl PeersTasks {
    ...
    pub fn sort_peers_for_blocks(&self, peers: &mut Vec<PeerIndex>) {
        peers.sort_by(|left, right| {
            let left_speed = self.stats.get(left).map(|s| s.speed.speed()).unwrap_or(
                0f64,
            );
            let right_speed = self.stats.get(right).map(|s| s.speed.speed()).unwrap_or(
                0f64,
            );
            // larger speed => better
            right_speed.partial_cmp(&left_speed).unwrap_or(
                Ordering::Equal,
            )
        })
    }
    ...
    pub fn on_block_received(&mut self, peer_index: PeerIndex, block_hash: &H256) {
        ...
        if !is_last_requested_block_received {
            let mut peer_blocks_requests = self.blocks_requests.remove(&peer_index).expect(
                "checked above; qed",
            );
    ...
    pub fn on_blocks_failure(&mut self, hashes: Vec<H256>) -> (Vec<H256>, Vec<H256>) {
        ...
        for hash in hashes {
            let is_failed_block = {
                let block_stats = self.blocks_stats.entry(hash.clone()).or_insert_with(
                    BlockStats::default,
                );
...
impl HeadersRequest {
    pub fn new() -> Self {
        HeadersRequest { timestamp: precise_time_s() }
    }
}
