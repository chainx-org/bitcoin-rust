...
use types::{PeerIndex, RequestId, StorageRef, MemoryPoolRef, PeersRef, ExecutorRef, ClientRef,
            ServerRef, SynchronizationStateRef, SyncListenerRef};
...
impl SimpleNode {
    ...
    pub fn get_block_template(&self) -> BlockTemplate {
        ...
        let previous_block_header = self.storage
        ...
        let max_block_size = self.consensus.fork.max_block_size(
            new_block_height,
            median_timestamp,
        );
        let block_assembler = BlockAssembler {
            max_block_size: max_block_size as u32,
            max_block_sigops: self.consensus.fork.max_block_sigops(
                new_block_height,
                max_block_size,
            ) as u32,
        };
    ...
    V: Client,
{
    ...
    pub fn on_getheaders(&self, peer_index: PeerIndex, message: types::GetHeaders, id: RequestId) {
        if self.state.synchronizing() {
            ...
            self.executor.execute(
                SynchronizationTask::Ignore(peer_index, id),
            );
            return;
        }
        ...
        self.client.after_peer_nearly_blocks_verified(
            peer_index,
            Box::new(lazy_server_task),
        );
    }
    ...
    pub fn on_sendheaders(&self, peer_index: PeerIndex, _message: types::SendHeaders) {
        ...
        self.peers.set_block_announcement_type(
            peer_index,
    ...
    pub fn on_merkleblock(&self, peer_index: PeerIndex, _message: types::MerkleBlock) {
        ...
        self.peers.misbehaving(
            peer_index,
            "Got unrequested 'merkleblock' message",
        );
    }
    ...
    pub fn on_compact_block(&self, peer_index: PeerIndex, _message: types::CompactBlock) {
        ...
        self.peers.misbehaving(
            peer_index,
            "Got unrequested 'cmpctblock' message",
        );
    }
    ...
    pub fn on_block_txn(&self, peer_index: PeerIndex, _message: types::BlockTxn) {
        ...
        self.peers.misbehaving(
            peer_index,
            "Got unrequested 'blocktxn' message",
        );
    }
...
pub mod tests {
    ...
    fn local_node_serves_block() {
        ...
        local_node.on_getdata(peer_index, types::GetData { inventory: inventory.clone() });
        // => `getdata` is served
        let tasks = server.take_tasks();
        assert_eq!(
            tasks,
            vec![
                ServerTask::GetData(peer_index, types::GetData::with_inventory(inventory)),
            ]
        );
    }
