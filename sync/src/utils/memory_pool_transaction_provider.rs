...
impl MemoryPoolTransactionOutputProvider {
    ...
    pub fn for_transaction(
        ...
        match check_result {
            ...
            DoubleSpendCheckResult::DoubleSpend(_, hash, index) => Err(
                TransactionError::UsingSpentOutput(hash, index),
            ),
                ...
                mempool_inputs: transaction
                    ...
                    .map(|input| {
                        (
                            ...
                            memory_pool.transaction_output(
            ...
            DoubleSpendCheckResult::NonFinalDoubleSpend(nonfinal_spends) => Ok(
...
impl TransactionOutputProvider for MemoryPoolTransactionOutputProvider {
    fn transaction_output(
        ...
        self.storage_provider.transaction_output(
            prevout,
            transaction_index,
        )
    }
    ...
    fn is_spent(&self, prevout: &OutPoint) -> bool {
        ...
        if let Some(ref nonfinal_spends) = self.nonfinal_spends {
            if nonfinal_spends.double_spends.contains(
                &prevout.clone().into(),
            )
            {
                return false;
