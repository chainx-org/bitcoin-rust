...
use utils::{KnownHashFilter, KnownHashType, BloomFilter, FeeRateFilter, build_compact_block,
            build_partial_merkle_tree};
...
impl ConnectionFilter {
    ...
    pub fn filter_transaction(
        ...
        self.known_hash_filter.filter_transaction(&transaction.hash) &&
            self.fee_rate_filter.filter_transaction(
    ...
    pub fn build_compact_block(&self, block: &IndexedBlock) -> types::CompactBlock {
        let unknown_transaction_indexes = block
            ...
            .enumerate()
            .filter(|&(_, tx)| {
                self.known_hash_filter.contains(
                    &tx.hash,
            ...
            .map(|(idx, _)| idx)
            .collect();
        types::CompactBlock { header: build_compact_block(block, unknown_transaction_indexes) }
    }
    ...
    pub fn build_merkle_block(&self, block: &IndexedBlock) -> Option<MerkleBlockArtefacts> {
        ...
        result.merkleblock.flags = partial_merkle_tree
            ...
            .into_iter()
            .map(|b| {
                ((b & 0b10000000) >> 7) | ((b & 0b01000000) >> 5) | ((b & 0b00100000) >> 3) |
                    ((b & 0b00010000) >> 1) |
                    ((b & 0b00001000) << 1) | ((b & 0b00000100) << 3) |
                    ((b & 0b00000010) << 5) | ((b & 0b00000001) << 7)
            })
            .collect::<Vec<u8>>()
            .into();
        Some(result)
...
pub mod tests {
    ...
    fn filter_default_accepts_block() {
        assert!(ConnectionFilter::default().filter_block(
    ...
    fn filter_rejects_transaction_known() {
        ...
        assert!(filter.filter_transaction(
            &test_data::block_h2().transactions[0].clone().into(),
            None,
        ));
    }
    ...
    fn filter_rejects_transaction_bloomfilter() {
        ...
        filter.add(types::FilterAdd { data: (&*tx.hash as &[u8]).into() });
        assert!(filter.filter_transaction(&tx, None));
        filter.clear();
        assert!(filter.filter_transaction(&tx, None));
    }
