    ...
    pub fn contains(&self, data: &[u8]) -> bool {
        for hash_function_idx in 0..self.hash_functions_num {
            let murmur_seed = hash_function_idx
                ...
                .0;
            let murmur_hash = murmur3_32(&mut data.as_ref(), murmur_seed) as usize %
                self.filter.len();
            let index = (murmur_hash & !7usize) | ((murmur_hash & 7) ^ 7);
            if !self.filter.get(index).expect(
                "murmur_hash is result of mod operation by filter len; qed",
            )
            {
                return false;
            }
