...
pub fn parse(matches: &clap::ArgMatches) -> Result<Config, String> {
    let db_cache = match matches.value_of("db-cache") {
        Some(s) => {
            s.parse().map_err(|_| {
                "Invalid cache size - should be number in MB".to_owned()
            })?
        }
    ...
    let user_agent = match network {
        ...
        Network::Mainnet |
        Network::Unitest |
        Network::Other(_) => format!("{}{}", USER_AGENT, user_agent_suffix),
    ...
    let seednodes: Vec<String> = match matches.value_of("seednode") {
        ...
        None => {
            match (network, &consensus.fork) {
                (Network::Mainnet, &ConsensusFork::BitcoinCash(_)) => {
                    bitcoin_cash_seednodes()
                        .into_iter()
                        .map(Into::into)
                        .collect()
                }
                (Network::Testnet, &ConsensusFork::BitcoinCash(_)) => {
                    bitcoin_cash_testnet_seednodes()
                        .into_iter()
                        .map(Into::into)
                        .collect()
    ...
    let host = match matches.value_of("host") {
        Some(s) => {
            s.parse::<net::IpAddr>().map_err(
                |_| "Invalid host".to_owned(),
            )?
        }
        None => {
            match only_net {
    ...
    let block_notify_command = match matches.value_of("blocknotify") {
        Some(s) => Some(s.parse().map_err(
            |_| "Invalid blocknotify commmand".to_owned(),
        )?),
        None => None,
    };
    ...
    let verification_edge = match matches.value_of("verification-edge") {
        Some(s) if verification_level != VerificationLevel::Full => {
            let edge: H256 = s.parse().map_err(
                |_| "Invalid verification edge".to_owned(),
            )?;
            edge.reversed()
        }
...
fn parse_consensus_fork(
    ...
    let new_consensus_fork = match (matches.is_present("btc"), matches.is_present("bch")) {
        ...
        _ => {
            return Err(
                "You can only pass single fork argument: --btc, --bch".into(),
            )
        }
    };
...
fn parse_rpc_config(network: Network, matches: &clap::ArgMatches) -> Result<RpcHttpConfig, String> {
    ...
    if let Some(cors) = matches.value_of("jsonrpc-cors") {
        config.cors = Some(vec![
            cors.parse().map_err(
                |_| "Invalid JSON RPC CORS".to_owned()
            )?,
        ]);
    }
    if let Some(hosts) = matches.value_of("jsonrpc-hosts") {
        config.hosts = Some(vec![
            hosts.parse().map_err(
                |_| "Invalid JSON RPC hosts".to_owned()
            )?,
        ]);
    }
