...
use core_rpc::{Server, start_http, MetaIoHandler, Compatibility, Remote};
use network::Network;
use std::io;
use sync;
use storage;
...
pub fn new_http(conf: HttpConfiguration, deps: Dependencies) -> Result<Option<Server>, String> {
    ...
    let addr = try!(url.parse().map_err(|_| {
        format!("Invalid JSONRPC listen host/port given: {}", url)
    }));
...
pub fn setup_http_rpc_server(
    ...
    match start_result {
        Err(ref err) if err.kind() == io::ErrorKind::AddrInUse => {
            Err(format!(
                "RPC address {} is already in use, make sure that another instance of a Bitcoin node is not running or change the address using the --jsonrpc-port and --jsonrpc-interface options.",
                url
            ))
        }
