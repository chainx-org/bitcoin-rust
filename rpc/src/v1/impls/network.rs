    ...
    T: NetworkApi,
{
    fn add_node(&self, node: String, operation: AddNodeOperation) -> Result<(), Error> {
        let addr = try!(node.parse().map_err(|_| {
        ...
        match operation {
            AddNodeOperation::Add => {
                self.api.add_node(addr).map_err(
                    |_| errors::node_already_added(),
                )
            }
            AddNodeOperation::Remove => {
                self.api.remove_node(addr).map_err(
                    |_| errors::node_not_added(),
                )
            }
    ...
    fn node_info(&self, _dns: bool, node_addr: Trailing<String>) -> Result<Vec<NodeInfo>, Error> {
        ...
        Ok(match node_addr {
            ...
            Some(node_addr) => {
                let addr = try!(node_addr.parse().map_err(|_| {
                    errors::invalid_params(
                        "node",
                        "Invalid ip address format, should be ip address (127.0.0.1)",
                    )
                }));
                let node_info = try!(self.api.node_info(addr).map_err(
                    |_| errors::node_not_added(),
                ));
                vec![node_info]
            }
...
impl NetworkApi for NetworkClientCore {
    ...
    fn node_info(&self, node_addr: IpAddr) -> Result<NodeInfo, p2p::NodeTableError> {
        ...
        let peers: Vec<p2p::PeerInfo> = self.p2p
            .connections()
            .info()
            .into_iter()
